  ...
  try {
    ...
    upload(req, res, async (err) => {
      ...
      if (discountRate < 0 || discountRate > 100) {
        return res
          .status(400)
          .json({ message: "Discount rate must be between 0 and 100" });
      }
      ...
      try {
        ...
        return res
          .status(400)
          .json({ message: "Invalid applicableProducts format" });
      }

      if (productIds.length === 0) {
        return res
          .status(400)
          .json({ message: "At least one applicable product is required" });
      }
      ...
      const promotionReferences = products.map((product) => {
        ...
        if (!seller) {
          throw new Error(
            `Seller ${sellerId} not found for product ${product._id}`
          );
        }
      ...
      const updatePromises = promotionReferences.map(
        ({ productId, promotionRef }) => {
  ...
  try {
    ...
    const promotion = await Promotion.findById(promotionId)
      .populate({
        path: "applicableProducts",
        select:
          "name price reference images description categoryDetails sellers",
        populate: [
          { path: "categoryDetails.category", select: "name" },
          {
