...
exports.approveApplication = async (req, res, next) => {
  try {
    ...
    if (!user || (user.role !== "seller" && user.role !== "delivery")) {
      return res
        .status(404)
        .json({ message: "User not found or not eligible for approval" });
    }
...
exports.rejectApplication = async (req, res, next) => {
  try {
    ...
    if (!user || (user.role !== "seller" && user.role !== "delivery")) {
      return res
        .status(404)
        .json({ message: "User not found or not eligible for rejection" });
    }
...
exports.suspendUser = async (req, res, next) => {
  try {
    ...
    if (!user || (user.role !== "seller" && user.role !== "delivery")) {
      return res
        .status(404)
        .json({ message: "User not found or not eligible for suspension" });
    }
  ...
  try {
    ...
    if (!user || (user.role !== "seller" && user.role !== "delivery")) {
      return res
        ...
        .json({
          message:
            "User not found or not eligible for cancellation of suspension",
        });
    }
    ...
    res.json({
      message: "User suspension canceled successfully",
      userId: user._id,
    });
  } catch (error) {
    next(error);
