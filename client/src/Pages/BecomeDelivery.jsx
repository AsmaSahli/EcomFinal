...
const BecomeDelivery = () => {
    ...
    if (files) {
      if (files[0].type !== "application/pdf") {
        setAlert({
          show: true,
          type: "error",
          message: t("delivery.fileTypeError"),
        });
        return;
      }
      if (files[0].size > 5 * 1024 * 1024) {
        setAlert({
          show: true,
          type: "error",
          message: t("delivery.fileSizeError"),
        });
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "http://localhost:8000/api/delivery/register",
        formDataToSend,
        {
          headers: {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-50 to-purple-50 p-6">
      {alert.show && (
        <div
          className={`fixed top-4 right-4 w-96 z-50 alert ${
