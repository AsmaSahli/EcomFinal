    ...
    if (files) {
      if (files[0].type !== "application/pdf") {
        setAlert({
          show: true,
          type: "error",
          message: t("seller.fileTypeError"),
        });
        return;
      }
      if (files[0].size > 5 * 1024 * 1024) {
        setAlert({
          show: true,
          type: "error",
          message: t("seller.fileSizeError"),
        });
        return;
      }
      ...
      {alert.show && (
        <div
          className={`alert alert-${
            alert.type
          } fixed top-4 right-4 w-96 z-50 shadow-lg rounded-lg transition-all duration-300 transform ${
            alert.show
