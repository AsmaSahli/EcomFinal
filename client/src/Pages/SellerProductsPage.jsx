...
const SellerProductsPage = () => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/products/sellers/${sellerId}/products`
        );
        ...
        setError(
          error.response?.data?.message || t("sellerProductsPage.error")
        );
        setProducts([]);
      } finally {
        setLoading(false);
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message ||
          t("sellerProductsPage.wishlistMessages.updateError")
      );
    }
  };
  ...
  if (products.length === 0) {
    return (
      <div className="container mx-auto p-4">
        {t("sellerProductsPage.noProducts")}
      </div>
    );
  }
      ...
      {products.length > 0 ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">
            {t("sellerProductsPage.emptyState")}
          </p>
        </div>
      )}
    </div>
